...
const FullProductsList = () => {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    // Sort logic within the useEffect for efficient updates
...
const scrollRight = () => {
  ...
  return (
    ...
    : sortedProducts.length > 1 && sortedProducts.slice(start, start + 4).map((product, relativeIndex) => (
        <ProductCard key={start + relativeIndex} product={product} addToCart={addToCart} navigateToSingleProductView={navigateToSingleProductView} />
    ))
}
